...
int main()
{
  ...
  char filename[64];
  sprintf(filename, "ofdm_rrc_setup_fft%u", nof_sc);
  ...
  for (uint32_t i = 0; i < data->N_bytes; i++) {
    oss << "0x" << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(data->msg[i]) << ", ";
  }
  logger.info("Message: %s", oss.str().c_str());
