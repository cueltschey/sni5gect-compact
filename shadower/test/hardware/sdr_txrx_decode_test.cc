...
std::string       source_param = "type=x300,master_clock_rate=184.32e6,clock=gpsdo,sync=gpsdo,sampling_rate=23.04e6";
                     ...
                     uint32_t              slot_len)
{
  ...
  while (running) {
    ...
    srsran_vec_cf_copy(ue_dl_buffer, task->dl_buffer[0]->data(), slot_len);
...
{
  int opt;
  while ((opt = getopt(argc, argv, "pPr")) != -1) {
    switch (opt) {
      case 'p':
